...
export class HeatmapComponent implements OnInit {
  ...
  async onLoadHeatmap(){
    ...
    for (let index = 0; index < lati.length; index++) {
      const element = lati[index];
      const element1 = longi[index];
      const element2 = rssi[index];

      // console.log(Math.abs(index));
      LatLng = [
        {location: new google.maps.LatLng(lati[index-10], longi[index-15]),weight:Math.abs(70)}, //มันเอาชุดข้อมูลตัวสุดท้ายไปใช้
        {location: new google.maps.LatLng(lati[index], longi[index]),weight:Math.abs(rssi[index])}
      ];
  ...
  async onMapLoad(mapInstance:  google.maps.Map,LatLng) {
    ...
    console.log('LatLng ',this.heatmap);
    this.heatmap.set('gradient', this.heatmap.get('gradient') ? null : gradient);
